...
from pose_detector import (
    detect_pose,
    reload_models,  # 重要：导入reload_models函数
    PoseDetectionError,
    NoKeypointError,
    InvalidPoseError,
    get_supported_poses,
...
class AppConfig:
    ...
    COS_CDN_DOMAIN: str = os.environ.get('COS_CDN_DOMAIN', '')
    ...
    LOG_FILE: str = os.environ.get('LOG_FILE', 'pose_detection.log')
...
class RequestIdFilter(logging.Filter):
    ...
    def filter(self, record):
        try:
            ...
            record.request_id = 'NO-REQUEST-ID-EXCEPTION'
        return True
...
def setup_logging(config_obj: AppConfig):
    ...
    logging.basicConfig(
        level=getattr(logging, config_obj.LOG_LEVEL, logging.INFO),
...
def before_request_hook():
    ...
    logger.info(
        f"Incoming request: {request.method} {request.full_path} from {get_client_ip()} "
...
@track_metrics('detect_pose_file_route')
def detect_pose_file_route():
    ...
    try:
        ...
        return jsonify(ResponseBuilder.error("FILE_READ_ERROR", "读取上传文件内容失败。", pose_id=pose_id)), 500
    ...
    try:
        ...
        skeleton_url = upload_buffer(skeleton_buffer, cos_key)
        ...
        return jsonify(
            ResponseBuilder.success(
                ...
                pose_id=pose_id,
                processingTimeMs=processing_times,
            )
        ), 200
...
@track_metrics('health_check_route')
def health_check_route():
    ...
    try:
        ...
        detector_check["status"] = "unhealthy"
...
def api_info_route():
    ...
    return jsonify({
        ...
        "documentation_url": "/swagger-ui" if config.DEBUG else "请查阅API文档或联系技术支持获取详细信息。",
        "contact_info": {"support_email": "tech-support@example.com"},
        "endpoints_overview": [
            ...
            {"path": "/api/reload_model", "method": "POST", "summary": "热重载模型权重（生产环境需验证）。"}
        ],
        "system_capabilities": {
            ...
            "model_hot_reload_enabled": True  # 新增特性
        }
    })
...
@track_metrics('reload_model_route')
def api_reload_model():
    """
    热重载模型端点
    注意：生产环境建议增加认证机制，防止滥用
    """
    try:
        logger.info(f"收到模型热重载请求，来源IP: {get_client_ip()}")
        
        # 调用 pose_detector 中的 reload_models 函数
        ...
        if result.get("status") == "success":
            ...
            response["msg"] = "模型部分重载成功"
            return jsonify(response), 207  # 207 Multi-Status
        else:
            response["code"] = "RELOAD_FAILED"
            response["msg"] = "模型重载失败"
